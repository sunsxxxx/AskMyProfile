    ...
    <div class="stage">
      ...
      <section ref="scrollArea" class="conversation" aria-live="polite">
        ...
        <div v-else class="message-list">
          ...
          />
          <div v-if="isStreaming && status" class="status"><i></i><i></i><i></i>{{ status }}</div>
        </div>
      </section>
      ...
      <footer class="composer-wrap">
        <div class="composer" :class="{ 'composer--active': input.trim() }">
          ...
          <button v-else class="send-button" type="button" :disabled="!canSend" aria-label="发送" @click="send()"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 19V5M7 10l5-5 5 5" /></svg></button>
        </div>
...
<style>
:root { font-family: Inter, "PingFang SC", "Microsoft YaHei", system-ui, sans-serif; color: #0d0e10; background: #f3f7ff; font-synthesis: none; }
* { box-sizing: border-box; }
html, body, #app { width: 100%; min-width: 320px; height: 100%; margin: 0; }
body { overflow: hidden; }
button, textarea { font: inherit; } button { color: inherit; }
.shell { height: 100%; height: 100dvh; padding: clamp(12px, 3.2dvh, 34px) clamp(12px, 2.5vw, 40px) clamp(10px, 2.8dvh, 30px); background: radial-gradient(circle at 58% 44%, rgba(255,255,255,.98) 0 14%, rgba(240,246,255,.82) 44%, rgba(225,235,254,.88) 74%, rgba(248,249,251,.95) 100%); }
...
.conversation { min-height: 0; overflow-y: auto; overscroll-behavior: contain; scrollbar-width: thin; scrollbar-color: #d6dce8 transparent; }
.shell:not(.shell--chat) .conversation { display: flex; overflow: hidden; }
.welcome-card { width: 100%; max-height: min(590px, 100%); margin: auto 0; display: flex; flex-direction: column; justify-content: center; padding: clamp(18px, 4.9dvh, 46px) clamp(22px, 5.2vw, 64px) clamp(16px, 4.4dvh, 42px); border: 1px solid rgba(221,226,237,.76); border-radius: clamp(22px, 2vw, 34px); background: rgba(255,255,255,.83); box-shadow: 0 18px 54px rgba(64,89,134,.10); backdrop-filter: blur(18px); }
...
.composer-wrap { position: relative; }.composer { min-height: clamp(60px, 8.7dvh, 82px); display: flex; align-items: center; gap: 13px; border: 1px solid #e0e4ec; border-radius: 999px; padding: clamp(6px, 1.05dvh, 10px) 16px clamp(6px, 1.05dvh, 10px) 20px; background: rgba(255,255,255,.92); box-shadow: 0 13px 38px rgba(45,71,118,.12); transition: border-color .18s, box-shadow .18s; }
.composer:focus-within { border-color: #bdc7d8; box-shadow: 0 0 0 4px rgba(105,128,169,.08), 0 15px 40px rgba(45,71,118,.14); }
...
.send-button { width: 52px; height: 52px; flex: 0 0 auto; display: grid; place-items: center; border: 0; border-radius: 50%; background: #050505; color: #fff; cursor: pointer; transition: transform .18s, opacity .18s; }.send-button svg { width: 23px; height: 23px; fill: none; stroke: currentColor; stroke-width: 1.7; stroke-linecap: round; stroke-linejoin: round; }.send-button:hover:not(:disabled) { transform: translateY(-2px); }.send-button:disabled { opacity: 1; cursor: not-allowed; }
...
.shell--chat { padding: clamp(20px, 2.7vh, 28px) 24px; }.shell--chat .stage { position: relative; grid-template-rows: 66px minmax(0, 1fr) auto; gap: 0; width: min(1128px, 100%); border: 1px solid rgba(221,226,237,.8); border-radius: 25px; padding: 0 26px 20px; background: rgba(255,255,255,.82); box-shadow: 0 18px 54px rgba(64,89,134,.10); backdrop-filter: blur(18px); overflow: hidden; }
.chat-toolbar { display: flex; justify-content: flex-end; align-items: center; }.new-button { display: flex; align-items: center; gap: 7px; border: 1px solid #e3e6ec; border-radius: 12px; padding: 9px 14px; background: rgba(255,255,255,.9); font-size: 12px; cursor: pointer; box-shadow: 0 3px 10px rgba(60,70,90,.08); transition: transform .18s, border-color .18s; }.new-button:hover:not(:disabled) { transform: translateY(-1px); border-color: #bcc4d1; }.new-button:disabled { opacity: .45; cursor: not-allowed; }
.message-list { padding: 4px 0 30px; }.status { max-width: 1058px; margin: 0 auto; padding: 4px 24px 14px 76px; display: flex; align-items: center; gap: 4px; color: #7a827d; font-size: 12px; }.status i { width: 4px; height: 4px; border-radius: 50%; background: #707988; animation: pulse 1.2s infinite; }.status i:nth-child(2) { animation-delay: .15s; }.status i:nth-child(3) { margin-right: 5px; animation-delay: .3s; }
.shell--chat .composer { min-height: 56px; padding: 6px 8px 6px 22px; border-radius: 999px; }.shell--chat .composer textarea { font-size: 14px; }.shell--chat .send-button { width: 43px; height: 43px; font-size: 24px; }
